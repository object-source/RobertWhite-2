...
class Ves_Megamenu_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    protected function _renderFormByFields( $fields, $data ){
        ...
        foreach( $fields as $widget => $field ){
            $output .= '<tr>';
            $output .=  '<td>'.Mage::helper("ves_megamenu")->__('Widget '.$widget).'</td>';
            $input = '';
            ...
            switch( $field['type']  ){
                case 'tree':
                    $html = "";
                    $input .='    <select id="category" class="myinput-text required-entry widthinput" name="params['.$widget.']">';

                    $input .= $this->genCategoriesOption( $val );
                    $input .= '</select>';
                    break;
                case 'static_blocks':
                    $html = "";
                    $input .='    <select id="static_block" class="myinput-text required-entry widthinput" name="params['.$widget.']">';
                ...
                case 'page_blocks':
                    $html = "";
                    $input .='    <select id="page_block" class="myinput-text required-entry widthinput" name="params['.$widget.']">';
    ...
    function genCategoriesOption($select  = 0){
        ...
        foreach($collection as $option) {
            $type = Mage::getModel("catalog/category")->load($option->getId());
            if($type->getData( "name" )) {
                if($select) {
                    $selectOption = '';
                    if($option->getId() == $select)
                        $selectOption = 'selected="selected"';
                    $html .= '<option value="'.$option->getId().'" '.$selectOption.'>'.$type->getData( "name" ).'</option>';
                } else {
                    $html .= '<option value="'.$option->getId().'">'.$type->getData( "name" ).'</option>';
                }
            }
        }
        return $html;
