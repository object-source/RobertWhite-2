...
class Ebizmarts_SagePaySuite_Model_Api_Payment extends Mage_Payment_Model_Method_Cc {
    ...
    protected function _setRequestCurrencyAmount($request, $quote) {
        ...
        if ($trnCurrency == 'store') {
            $request->setAmount($this->formatAmount($quote2->getGrandTotal()-$this->_getSessionSurcharge(), $quote2->getQuoteCurrencyCode()));
            ...
            $request->setAmount($this->formatAmount($quote2->getGrandTotal()-$this->_getSessionSurcharge(), $currencyCode));
            $request->setCurrency($currencyCode);
        }
        else {
            $request->setAmount($this->formatAmount($quote2->getBaseGrandTotal()-$this->_getSessionSurcharge(), $quote2->getBaseCurrencyCode()));
    ...
    public function getSurchargeXml() {
        ...
        $_config = $this->getConfigData('surcharge_creditcards');
        ...
        if ($config && count($config)) {
            ...
            foreach ($config as $rule) {
                ...
                $nodi->addChild($rule['chargetype'], number_format($rule['amount'], 2, '.', ''));
            }
