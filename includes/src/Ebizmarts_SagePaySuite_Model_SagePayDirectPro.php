...
class Ebizmarts_SagePaySuite_Model_SagePayDirectPro extends Ebizmarts_SagePaySuite_Model_Api_Payment {
    ...
    public function completePayPalTransaction(array $request, $quote) {
        ...
        if ($trnCurrency == 'store') {
            $pdata ['Amount'] = $this->formatAmount($quote->getGrandTotal()-$this->_getSessionSurcharge(), $quote->getCurrencyCode());
        }
        else if ($trnCurrency == 'switcher') {
            $pdata ['Amount'] = $this->formatAmount($quote->getGrandTotal()-$this->_getSessionSurcharge(), Mage::app()->getStore()->getCurrentCurrencyCode());
        }
        else {
            $pdata ['Amount'] = $this->formatAmount($quote->getBaseGrandTotal()-$this->_getSessionSurcharge(), $quote->getQuoteCurrencyCode());
        }
    ...
    public function directCallBack3D(Varien_Object $payment, $PARes, $MD) {
        ...
        //Used in surcharges to retrieve amount
    ...
    public function registerTransaction($params = null, $onlyToken = false, $macOrder = null) {
        ...
        if (is_null($macOrder)) {
            $amount = $this->formatAmount($quoteObj2->getGrandTotal()-$this->_getSessionSurcharge(), $quoteObj2->getCurrencyCode());
        }
        else {

            $amount = $this->formatAmount($macOrder->getGrandTotal()-$this->_getSessionSurcharge(), $macOrder->getCurrencyCode());

            $baseAmount = $this->formatAmount($macOrder->getBaseGrandTotal()-$this->_getSessionSurcharge(), $macOrder->getQuoteCurrencyCode());
