...
class Magestore_Webpos_Model_Cart
{
	...
	public function checkoutCartAdd($observer){
		...
		if ($action->getRequest()->getParam('iswebposcart') == 'true'){
			...
			} elseif ($params['groupmessage'] || $params['minicart'] || $params['webposlinks'] || $params['iswebposcartpage']){
				$action->loadLayout();
				try {
		...
		if ($action->getRequest()->getParam('iswebposcart') == 'true'
			...
			if ($cartBlock = $action->getLayout()->getBlock('checkout.cart')){
                            ...
                            $cartHtml = $cartBlock
                                    ...
                                    ->addItemRender('downloadable', $nameModule.'/cart_item_renderer_downloadable', $template)
                                    ->toHtml();
